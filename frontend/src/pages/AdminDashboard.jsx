      ...
      <AdminLayout>
        <div className="flex items-center justify-center h-[60vh]">
          <div className="text-center space-y-4">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Initializing System...</p>
          </div>
        </div>
      </AdminLayout>
    )
      ...
      <div className="space-y-10 pb-10">
        <div className="flex flex-col gap-2">
          ...
          <h2 className="font-display text-4xl font-black text-white">
            System <span className="text-zinc-600">Health</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          {/* AI Usage by Type */}
          <div className="xl:col-span-2 card p-8 border-white/5 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-xl font-bold text-white">AI Intelligence Breakdown</h3>
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Last 7 Days</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {stats?.aiUsageStats.map(item => (
                <div key={item._id} className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-amber-500/30 transition-all group">
          ...
          <div className="card p-8 border-white/5 bg-amber-500/[0.02] flex flex-col justify-between">
             <div className="space-y-2">
                <h3 className="font-display text-xl font-bold text-white">Quick Actions</h3>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest leading-relaxed">Common administrative tasks and system controls.</p>
             </div>
             <div className="space-y-3 mt-8">
                <button className="w-full py-4 rounded-xl bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-white hover:border-amber-500/50 hover:text-amber-500 transition-all text-left px-5 flex items-center justify-between group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="card p-8 border-white/5">
              <div className="flex items-center justify-between mb-8">
                <h3 className="font-display text-xl font-bold text-white">Latest Onboarding</h3>
              ...
              <div className="space-y-4">
                {recentActivities.users.map(u => (
                  <div key={u._id} className="p-4 rounded-xl flex items-center justify-between bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-xs text-white uppercase tracking-wider">{u.name}</p>
                        <p className="text-[10px] font-medium text-zinc-500">{u.email}</p>
                      </div>
                    </div>
                    <span className={`text-[9px] font-bold px-2 py-1 rounded uppercase tracking-widest ${u.role === 'admin' ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'bg-white/5 text-zinc-500'}`}>
                      {u.role}
                    </span>
                  </div>
                ))}
            ...
            <div className="card p-8 border-white/5">
              <div className="flex items-center justify-between mb-8">
                <h3 className="font-display text-xl font-bold text-white">Recent Artifacts</h3>
              ...
              <div className="space-y-4">
                {recentActivities.resumes.map(r => (
                  <div key={r._id} className="p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group">
                    <div className="flex items-center justify-between">
                      <p className="font-bold text-xs text-white uppercase tracking-wider">{r.title}</p>
                      <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest">{new Date(r.createdAt).toLocaleDateString()}</span>
                    </div>
...
function StatCard({ icon, label, value, color }) {
  return (
    <div className="card p-8 border-white/5 relative overflow-hidden group">
