...
function ResumeCard({ resume, onDelete, onDuplicate }) {
  ...
  const statusColor = score >= 70 ? 'text-emerald-500' : score >= 40 ? 'text-amber-500' : 'text-red-500'

  return (
    <div
      className="p-4 rounded-xl bg-white/[0.02] border border-border flex flex-col sm:flex-row sm:items-center justify-between group hover:border-amber-500/30 transition-all cursor-pointer"
      ...
      <div className="mb-4 sm:mb-0">
        ...
        <div className="flex items-center gap-4">
          <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
            Template: <span className="text-zinc-400">{resume.template || 'Modern'}</span>
          </p>

          <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
            ATS Score: <span className={statusColor}>{score}%</span>
          </p>
        </div>
      </div>

      <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
        <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-600 mr-2 hidden md:block">
          {formatDistanceToNow(new Date(resume.updatedAt), { addSuffix: true })}
        </span>

        <button
          onClick={() => onDuplicate(resume._id)}
          className="w-8 h-8 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 flex items-center justify-center text-zinc-400 hover:text-white transition-all"
          title="Duplicate"
        >
        ...
        <button
          ...
          className="w-8 h-8 rounded-lg bg-red-500/5 border border-red-500/10 hover:bg-red-500/10 flex items-center justify-center text-red-500/60 hover:text-red-500 transition-all"
          title="Delete"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
        ...
        <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-600 group-hover:bg-amber-500/10 group-hover:text-amber-500 transition-colors ml-1">
          →
        </div>
      </div>
    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-12">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-fade-up delay-200">
              {[
                ...
                <Link
                  key={to}
                  to={to}
                  className="card p-6 flex items-center gap-5 hover:border-amber-500/30 group"
                >
            ...
            <div className="animate-fade-up delay-300">
              ...
              <div className="card p-6 bg-white/[0.02] overflow-y-auto custom-scrollbar" style={{ maxHeight: 'calc(76vh - 55px)' }}>
                {loading ? (
                  ...
                  <div className="p-10 text-center bg-white/[0.01] border border-dashed border-border rounded-xl">
                    <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mx-auto mb-4">
                      <svg className="w-6 h-6 text-zinc-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                      </svg>
                    </div>
                    <h4 className="text-sm text-text-primary font-bold mb-1">No resumes found</h4>
                    <p className="text-[10px] text-zinc-600 uppercase tracking-widest mb-6">Start by choosing a professional template.</p>
                    <button onClick={handleCreate} className="btn-primary text-[10px] px-6">Start Building</button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {resumes.map(resume => (
          ...
          <div className="lg:col-span-4 space-y-8 animate-fade-up delay-400 sticky top-28 self-start pb-10">
            ...
            <div className="card p-6 bg-white/[0.02]">
              ...
              <div className="space-y-4">
                {tests.length === 0 ? (
                  <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-700 py-4 text-center">
                  ...
                  tests.slice(0, 3).map(test => (
                    <div
                      key={test._id}
                      className="p-4 rounded-xl bg-white/[0.02] border border-border flex items-center justify-between group hover:border-white/10 transition-all cursor-pointer"
                      onClick={() => navigate(`/resume-test`)}
                    >
                      <div>
                        <p className="text-xs font-bold text-text-primary mb-1 line-clamp-1">
                          {test.title || 'Skills Assessment'}
                        </p>

                        <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">
                          Score:{' '}
                          <span className={test.score >= 70 ? 'text-emerald-500' : 'text-amber-500'}>
                            {test.score || 0}%
                          </span>
                        </p>
                      </div>
