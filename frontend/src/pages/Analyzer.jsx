          ...
          <div className="space-y-10 animate-fade-up">
              ...
              <div className="relative flex-shrink-0 animate-scale-in">
                <div className="w-44 h-44 rounded-full border-4 border-white/5 flex items-center justify-center relative">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="text-6xl font-black text-text-primary">{analysis.atsScore}</span>
                    <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">ATS Score</span>
                  </div>
                </div>
              </div>
              <div className="flex-1 text-center lg:text-left space-y-4">
                <h2 className="font-display text-3xl font-bold text-text-primary tracking-tight">Executive Summary</h2>
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-3 pt-4">
                   <div className="px-4 py-2 rounded-xl bg-white/[0.03] border border-white/5">
                      ...
                      <p className="text-sm font-black text-text-primary">{analysis.atsScore > 75 ? 'HIGH' : 'MEDIUM'}</p>
                   </div>
                   <div className="px-4 py-2 rounded-xl bg-white/[0.03] border border-white/5">
                      <p className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest mb-1">Keywords</p>
                      <p className="text-sm font-black text-text-primary">{analysis.keywords?.found.length || 0} / {(analysis.keywords?.found.length || 0) + (analysis.keywords?.missing.length || 0)}</p>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="card p-10 border-white/5 bg-white/[0.01]">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <h3 className="font-display text-2xl font-bold text-text-primary tracking-tight">Vocabulary Audit</h3>
            ...
            {analysis.suggestions?.length > 0 && (
              <div className="card p-10 border-white/5 bg-zinc-900/20">
                <div className="flex items-center gap-3 mb-10">
                  ...
                  <h3 className="font-display text-2xl font-bold text-text-primary tracking-tight">Strategic Fixes</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {analysis.suggestions.map((s, i) => (
                    <div key={i} className="p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-amber-500/30 transition-all group">
                      ...
                      <p className="text-base font-bold text-text-primary mb-2 group-hover:text-amber-500 transition-colors">{s.issue}</p>
