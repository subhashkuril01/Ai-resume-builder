...
const sanitizeTest = (testDoc) => {
  ...
  return {
    ...test,
    remainingSeconds: test.expiresAt && !test.submittedAt
...
const keywordScore = (answerText = '', expectedConcepts = []) => {
  ...
  const matched = expectedConcepts.filter((concept) => normalized.includes(String(concept).toLowerCase()));
  return {
...
const buildReportFromEvaluations = (test, questionEvaluations, aiReport = null, previousTest = null) => {
  ...
  const totalMaxScore = test.questions.reduce((sum, question) => sum + (question.maxPoints || 0), 0) || 1;
  ...
  const mistakeAnalysis = test.questions
    .map((question) => {
      ...
      return {
        ...
        explanation: evaluation.explanation || 'The selected option does not match the correct answer.',
        correctApproach: evaluation.correctApproach || question.correctAnswer?.explanation || 'Review the underlying concept.'
      };
    })
    .filter(Boolean);
  ...
  const strengths = aiReport?.strengths?.length
    ? aiReport.strengths
    : skillBreakdown.filter((item) => item.accuracy >= 75).map((item) => `${item.skill} fundamentals`);

  const report = {
    overallScore,
    accuracyLevel: aiReport?.accuracyLevel ?? overallScore,
    logicalThinkingScore: aiReport?.logicalThinkingScore ?? Math.max(35, Math.min(95, overallScore + 5)),
    problemSolvingAbility: aiReport?.problemSolvingAbility ?? Math.max(35, Math.min(95, overallScore + 3)),
    strengths,
    weakAreas,
    skillBreakdown: aiReport?.skillBreakdown?.length ? aiReport.skillBreakdown : skillBreakdown,
    mistakeAnalysis: aiReport?.mistakeAnalysis?.length ? aiReport.mistakeAnalysis : mistakeAnalysis,
  ...
  if (previousTest?.report) {
    const comparisonSkills = inferComparison(report.skillBreakdown, previousTest.report.skillBreakdown || []);
    report.comparison = {
...
const evaluateWithMockRules = (test, previousTest = null) => {
  const questionEvaluations = test.questions.map((question) => {
    ...
    return {
      ...
      correctApproach: question.correctAnswer.explanation || 'Review the underlying concept and compare each option carefully.',
...
const generateTestDocument = async ({ resume, userId, priorTests }) => {
  ...
  // Fallback: if no technical content, use soft skills or general topics from the resume
  if (!profile.technologiesUsed.length && !profile.technicalSkills.length && !profile.projects.length) {
    // Add fallback skills from soft skills, education, experience, or generic topics
    ...
    // If still empty, add generic fallbacks so the test can always be generated
    if (!profile.technicalSkills.length) {
      profile.technicalSkills.push('General Knowledge', 'Problem Solving', 'Communication', 'Critical Thinking');
    }
    profile.technologiesUsed.push(...profile.technicalSkills);
  }

  const attemptNumber = (priorTests[0]?.attemptNumber || 0) + 1;
  const priorPrompts = priorTests.flatMap((test) => test.questions.map((question) => question.prompt)).slice(0, 150);
...
const listTests = asyncHandler(async (req, res) => {
  const tests = await ResumeTest.find({ userId: req.user.id })
    .select('title resumeTitle resumeId status attemptNumber durationMinutes difficulty report.overallScore createdAt submittedAt')
    .sort({ createdAt: -1 })
    .limit(30);
...
const getTest = asyncHandler(async (req, res) => {
  ...
  if (test.status === 'in_progress' && test.expiresAt && new Date(test.expiresAt) <= new Date() && !test.submittedAt) {
    test.status = 'expired';
    await test.save();
  }
...
const saveAnswer = asyncHandler(async (req, res) => {
  ...
  question.userAnswer = {
    ...
    answerText: answerText !== undefined ? answerText : (question.userAnswer?.answerText || ''),
    timeSpentSeconds: Number.isFinite(timeSpentSeconds) ? timeSpentSeconds : (question.userAnswer?.timeSpentSeconds || 0),
    savedAt: new Date()
  };
  ...
  await test.save();
  res.json({ success: true, saved: true, remainingSeconds: sanitizeTest(test).remainingSeconds });
});
...
const submitTest = asyncHandler(async (req, res) => {
  ...
  test.questions = test.questions.map((question) => ({
    ...question.toObject(),
    evaluation: evaluationResult.questionEvaluations.find((item) => item.questionId === question.questionId) || question.evaluation
  }));
