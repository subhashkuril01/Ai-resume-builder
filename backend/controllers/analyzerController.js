...
const analyzeATS = asyncHandler(async (req, res) => {
  ...
  if (!resumeText.trim()) return res.status(400).json({ error: 'Resume content is empty. Please add content before analyzing.' });
  let analysisData;
  if (isMockMode()) {
...
const getAnalysisHistory = asyncHandler(async (req, res) => {
  ...
  if (!resume) return res.status(404).json({ error: 'Resume not found.' });
  const history = await AnalysisResult.find({ resumeId: req.params.resumeId, userId: req.user.id }).sort({ createdAt: -1 }).limit(10);
